      ...
      <main class="d-flex col-md-9 ms-sm-auto col-lg-10 px-md-4" role="main">
        <div class="container-fluid">
          <div v-for="(id, index) in list" :key="id" class="gene">
            <nuxt-link :to="'dna/' + id">{{listid[index]}}</nuxt-link>
          </div>
        </div>
      </main>
    </div>
...
export default {
  ...
  methods: {
    ...
    try {
      const res = await axios.get(`https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=genome&term=${text.replace(/ /g, '+')}&rettype=fasta&retmode=json&retmax=10`, config);
      let result = res.data.esearchresult.idlist;
      this.list = result;
      const rep = await axios.get(`https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=genome&id=${result}&rettype=fasta&retmode=json`, config);
      let tempos = []
