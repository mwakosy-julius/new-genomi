      ...
      <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
        <div class="position-sticky pt-3">
          <ul class="nav flex-column">
            ...
            <li class="nav-item">
              <nuxt-link class="nav-link" to="./gene">
                Genes
              </nuxt-link>
            </li>
            <li class="nav-item">
      ...
      <main class="d-flex col-md-9 ms-sm-auto col-lg-10 px-md-4" role="main">
        <div class="container-fluid">
          <div v-for="(id, index) in list" :key="id" class="gene">
            <nuxt-link :to="'protein/' + id">{{listid[index]}}</nuxt-link>
          </div> 
        </div>
      </main>
    </div>
...
import search from "./ProteinSearch.vue";
...
export default {
  ...
  methods: {
    ...
    try {
      const res = await axios.get(`https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=protein&term=${text.replace(/ /g, '+')}&rettype=fasta&retmode=json&retmax=10`, config);
      let result = res.data.esearchresult.idlist;
      this.list = result;
      const rep = await axios.get(`https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=protein&id=${result}&rettype=fasta&retmode=json`, config);
      let tempos = []
